...
const SAMPLE = `// Example (C++): nested loops
#include <bits/stdc++.h>
using namespace std;
...
const SAMPLE_SORT = `// Example (C++): sort call
#include <bits/stdc++.h>
using namespace std;

vector<int> sortedCopy(vector<int> arr) {
    sort(arr.begin(), arr.end());
    return arr;
}`;

const SAMPLE_LOG = `// Example (C++): logarithmic loop
#include <bits/stdc++.h>
using namespace std;
...
type ComplexityLanguage = (typeof LANGUAGE_OPTIONS)[number]["value"];

function ComplexityCalculatorPanel() {
  const [code, setCode] = React.useState(SAMPLE);
  const [language, setLanguage] = React.useState<ComplexityLanguage>("cpp");
    ...
    <Grid container spacing={2}>
      <Grid item xs={12} md={7}>
        ...
        >
          <Stack spacing={1.5}>
            ...
            >
              <Typography sx={{ color: "#DBD8E3" }}>Paste your code</Typography>
              <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
                ...
                >
                  ...
                  <Select
                    label="Language"
                    value={language}
                    onChange={(e) => setLanguage(e.target.value as ComplexityLanguage)}
                    sx={{
                      color: "#DBD8E3",
                      fontWeight: 600,
            ...
            >
              <Button
                ...
                onClick={() => {
                  setCode("");
                  setResult(
                    estimateComplexity("// Paste code above and click Analyze")
                  );
                }}
                sx={{
            ...
            <Typography sx={{ color: "rgba(219,216,227,0.72)", fontSize: 13 }}>
              Selected language:{" "}
      ...
      <Grid item xs={12} md={5}>
        ...
        >
          <Stack spacing={1.5}>
            <Box>
              <Typography sx={{ color: "#DBD8E3" }}>Estimated result</Typography>
            ...
            <Grid container spacing={1.25}>
              <Grid item xs={12}>
                ...
                >
                  ...
                  <Typography variant="h5" sx={{ color: "#DBD8E3", mt: 0.25 }}>
                    {result.time}
                  </Typography>
                </Paper>
              </Grid>
        ...
        <Stack spacing={2.5}>
          <Box>
            ...
            <Typography sx={{ color: "rgba(219,216,227,0.85)", mt: 0.5 }}>
              Estimate Big‑O for your code snippets in a quick and interactive way.
            </Typography>
          </Box>

          <ComplexityCalculatorPanel />
        </Stack>
      </Paper>
    </Box>
  );
