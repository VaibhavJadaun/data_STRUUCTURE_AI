...
function formatStructuredFallback(raw: string): string {
  return [
    "1. Definition (simple)",
    `- ${raw || "Not available."}`,
    "",
...
function ensureStructuredAnswer(content: string): string {
  const hasSections =
  ...
  if (hasSections && hasBullets) return content;
    ...
    if (isAxiosLike(error) && error.response) {
      ...
      if (status === 429) {
        return res.status(429).json({
          message:
            "Rate limit (429) after retries. If you use OpenAI, free tier limits are strict — add GROQ_API_KEY in .env (free tier at console.groq.com) or wait and try again.",
        });
      }
      if (status === 401) {
